...
public class Controller {
	...
	private void graphZeichnen() {
		...
		// setzte die Farben fuer Fill und Stroke auf Schwarz
		...
		for (int i = 0; i < knoten.size(); i++) {
			 ...
			 * 
			 * Das Minimum fuer die X-Position ist fuenf. Danach wird immer jeweils 50 zu der letzten X-Position hinzuaddiert.
			 */
			 ...
			 */
			int y_position_zaehlschritte = 400 / (knoten.get(i).size() + 1);
			...
			for (int j = 0; j < knoten.get(i).size(); j++) {
				// Ziehe einen Zaehlschritt von der Y-Position ab, damit die Y-Position die richtige Groesse hat.
				y_position -= y_position_zaehlschritte;
				// zeichne den Knoten
				gc.fillOval(x_position, y_position, 20, 20);
		...
		for (Kante kante : kanten) {
			/*
			 * Speichere die ID der aktuellen Knoten ab, um spaeter wieder darauf zuzugreifen zu koennen.
			 * 
			 * Die IDs wurden als Strings abgespeichert. Die waagerechten Ebenen und die senkrechten Ebenen wurden mit einem Punkt getrennt.
			 * 
			 * Um die ID fuer die spaetere Verwendung optimal abzuspeichern, wurden die beiden Teile der ID aufgeteilt und der Punkt entfernt.
			 */
			 ...
			 * 
			 * Die Zahlen aus den beiden Arrays "knoten_1" und "knoten_2" muessen um eins verkleinert werden, da die Zaehlung der IDs nicht bei null beginnt.
			 */
