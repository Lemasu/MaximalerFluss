...
public class GraphViewInitThread extends Thread {
	...
	public void run() {
		...
		while (controller.graphView.getWidth() == 0 || controller.graphView.getHeight() == 0) {			
			try {
				/*
				 * Es soll fuere fuer 100 Millisekunden gewartet werden, damit die Applikation nicht permanent ueberprueft, ob der SmartGraphPanel eine Groesse hat.
				 */
				Thread.sleep(100);
		...
		/*
		 * Initialisiere den SmartGraphPanel, damit die Labels von diesem spaeter aktualisiert werden koennen.
		 */
        ...
        for (int i = 0; i < controller.nodes.size(); i++) {
        	controller.graphView.setVertexPosition(controller.nodes.get(i), controller.x_positionen.get(i), controller.y_positionen.get(i));
        }
        ...
        for (int i = 0; i < controller.nodes.size(); i++) {
            ...
            controller.graphView.setVertexPosition(node, controller.x_positionen.get(i), controller.y_positionen.get(i));
        }
